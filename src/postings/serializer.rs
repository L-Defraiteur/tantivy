...
pub struct InvertedIndexSerializer {
    ...
    offsets_write: CompositeWrite<WritePtr>,
    schema: Schema,
}
...
impl InvertedIndexSerializer {
    ...
    pub fn open(segment: &mut Segment) -> crate::Result<InvertedIndexSerializer> {
        use crate::index::SegmentComponent::{Offsets, Positions, Postings, Terms};
        let inv_index_serializer = InvertedIndexSerializer {
            ...
            offsets_write: CompositeWrite::wrap(segment.open_write(Offsets)?),
    ...
    pub fn new_field(
        ...
        let offsets_write = self.offsets_write.for_field(field);
...
pub struct FieldSerializer<'a> {
    ...
    offsets_serializer_opt: Option<PositionSerializer<&'a mut CountingWriter<WritePtr>>>,
...
impl<'a> FieldSerializer<'a> {
    fn create(
        ...
        offsets_write: &'a mut CountingWriter<WritePtr>,
        fieldnorm_reader: Option<FieldNormReader>,
        ...
        let offsets_serializer_opt = if index_record_option.has_offsets() {
            Some(PositionSerializer::new(offsets_write))
        } else {
            None
        };
    ...
    fn current_term_info(&self) -> TermInfo {
        ...
        let offsets_start =
            if let Some(offsets_serializer) = self.offsets_serializer_opt.as_ref() {
                offsets_serializer.written_bytes()
            } else {
                0u64
            } as usize;
        ...
        TermInfo {
            ...
            offsets_range: offsets_start..offsets_start,
        }
    }
    ...
    /// Serialize a document with positions AND character byte offsets.
    ///
    /// `offset_from_deltas` and `offset_to_deltas` are delta-encoded like positions.
    /// Offsets are interleaved (from_0, to_0, from_1, to_1, ...) and written using
    /// the same bitpacked block encoding as positions.
    pub fn write_doc_with_offsets(
        &mut self,
        doc_id: DocId,
        term_freq: u32,
        position_deltas: &[u32],
        offset_from_deltas: &[u32],
        offset_to_deltas: &[u32],
    ) {
        self.current_term_info.doc_freq += 1;
        self.postings_serializer.write_doc(doc_id, term_freq);
        if let Some(ref mut positions_serializer) = self.positions_serializer_opt.as_mut() {
        ...
        if let Some(ref mut offsets_serializer) = self.offsets_serializer_opt.as_mut() {
            assert_eq!(term_freq as usize, offset_from_deltas.len());
            assert_eq!(term_freq as usize, offset_to_deltas.len());
            // Interleave: (from_0, to_0, from_1, to_1, ...)
            let mut interleaved = Vec::with_capacity(2 * term_freq as usize);
            for i in 0..term_freq as usize {
                interleaved.push(offset_from_deltas[i]);
                interleaved.push(offset_to_deltas[i]);
            }
            offsets_serializer.write_positions_delta(&interleaved);
        }
    }
    ...
    pub fn close_term(&mut self) -> io::Result<()> {
        ...
        if self.term_open {
            ...
            if let Some(offsets_serializer) = self.offsets_serializer_opt.as_mut() {
                offsets_serializer.close_term()?;
                self.current_term_info.offsets_range.end =
                    offsets_serializer.written_bytes() as usize;
            }
    ...
    pub fn close(mut self) -> io::Result<()> {
        ...
        if let Some(offsets_serializer) = self.offsets_serializer_opt {
            offsets_serializer.close()?;
        }
