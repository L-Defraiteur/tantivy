...
pub(crate) mod tests {
    ...
    #[test]
    pub fn test_offsets_round_trip() -> crate::Result<()> {
        let mut schema_builder = Schema::builder();
        let text_options = TextOptions::default().set_indexing_options(
            TextFieldIndexing::default()
        ...
        let index = Index::create_in_ram(schema);
        let mut index_writer = index.writer_for_tests()?;
        // "hello world" → tokens: "hello" (0..5), "world" (6..11)
        index_writer.add_document(doc!(text_field => "hello world"))?;
        // "abc be be be abc" → tokens: "abc" (0..3), "be" (4..6), "be" (7..9), "be" (10..12), "abc" (13..16)
        index_writer.add_document(doc!(text_field => "abc be be be abc"))?;
        index_writer.commit()?;
        ...
        // Test "hello": appears in doc 0 at position 0, offsets (0, 5)
        {
            let term = Term::from_field_text(text_field, "hello");
            let mut postings = inverted_index
                .read_postings(&term, IndexRecordOption::WithFreqsAndPositionsAndOffsets)?
                .unwrap();
            assert_eq!(postings.doc(), 0);
        ...
        // Test "world": appears in doc 0 at position 1, offsets (6, 11)
        {
            ...
            let mut offsets = Vec::new();
            postings.offsets(&mut offsets);
            assert_eq!(&offsets[..], &[(6, 11)]);
        }

        // Test "abc": appears in doc 1 at positions 0 and 4, offsets (0,3) and (13,16)
        {
            ...
            let mut offsets = Vec::new();
            postings.offsets(&mut offsets);
            assert_eq!(&offsets[..], &[(0, 3), (13, 16)]);
        }

        // Test "be": appears in doc 1 at positions 1,2,3, offsets (4,6),(7,9),(10,12)
        {
            let term = Term::from_field_text(text_field, "be");
            let mut postings = inverted_index
                .read_postings(&term, IndexRecordOption::WithFreqsAndPositionsAndOffsets)?
                .unwrap();
            assert_eq!(postings.doc(), 1);
            ...
            let mut offsets = Vec::new();
            postings.offsets(&mut offsets);
            assert_eq!(&offsets[..], &[(4, 6), (7, 9), (10, 12)]);
        }
