...
pub struct LoadedPostings {
    ...
    positions: Box<[u32]>,
    /// Byte offsets (offset_from, offset_to) per position, interleaved flat:
    /// [from_0, to_0, from_1, to_1, ...]. Same length layout as positions (via offset_offsets).
    offset_offsets: Box<[u32]>,
    byte_offsets: Box<[(u32, u32)]>,
    cursor: usize,
}
...
impl LoadedPostings {
    ...
    pub fn load(segment_postings: &mut SegmentPostings) -> LoadedPostings {
        ...
        LoadedPostings {
            ...
            offset_offsets: offset_offsets.into_boxed_slice(),
...
impl From<(Vec<DocId>, Vec<Vec<u32>>)> for LoadedPostings {
    fn from(doc_ids_and_positions: (Vec<DocId>, Vec<Vec<u32>>)) -> LoadedPostings {
        ...
        position_offsets.push(all_positions.len() as u32);
        let offset_offsets = vec![0u32; position_offsets.len()];
        LoadedPostings {
            doc_ids: doc_ids.into_boxed_slice(),
            positions: all_positions.into_boxed_slice(),
            position_offsets: position_offsets.into_boxed_slice(),
            offset_offsets: offset_offsets.into_boxed_slice(),
            byte_offsets: Box::new([]),
            cursor: 0,
        }
    }
}
...
impl Postings for LoadedPostings {
    ...
    fn append_offsets(&mut self, output: &mut Vec<(u32, u32)>) {
        let start = self.offset_offsets[self.cursor] as usize;
