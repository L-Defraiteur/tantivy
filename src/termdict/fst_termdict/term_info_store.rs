...
impl FixedSize for TermInfoBlockMeta {
    const SIZE_IN_BYTES: usize =
        u64::SIZE_IN_BYTES + TermInfo::SIZE_IN_BYTES + 4 * u8::SIZE_IN_BYTES;
}
...
impl TermInfoBlockMeta {
    ...
    fn deserialize_term_info(&self, data: &[u8], inner_offset: usize) -> TermInfo {
        ...
        let offsets_start_addr =
            positions_start_addr + self.positions_offset_nbits as usize;
        let offsets_end_addr = offsets_start_addr + num_bits;
        let doc_freq_addr = offsets_start_addr + self.offsets_offset_nbits as usize;
        ...
        let offsets_start_offset = self.ref_term_info.offsets_range.start
            + extract_bits(data, offsets_start_addr, self.offsets_offset_nbits) as usize;
        let offsets_end_offset = self.ref_term_info.offsets_range.start
            + extract_bits(data, offsets_end_addr, self.offsets_offset_nbits) as usize;
        ...
        TermInfo {
            ...
            offsets_range: offsets_start_offset..offsets_end_offset,
        }
    }
}
...
fn bitpack_serialize<W: Write>(
    ...
    bit_packer.write(
        term_info.offsets_range.start as u64,
        term_info_block_meta.offsets_offset_nbits,
        write,
    )?;
    bit_packer.write(
...
impl TermInfoStoreWriter {
    ...
    fn flush_block(&mut self) -> io::Result<()> {
        ...
        let offsets_end_offset =
            last_term_info.offsets_range.end - ref_term_info.offsets_range.start;
        for term_info in &mut self.term_infos[1..] {
            ...
            term_info.offsets_range.start -= ref_term_info.offsets_range.start;
        }
        ...
        let max_offsets_offset_nbits = compute_num_bits(offsets_end_offset as u64);
        ...
        let term_info_block_meta = TermInfoBlockMeta {
            ...
            offsets_offset_nbits: max_offsets_offset_nbits,
        };
        ...
        // We still need to serialize the end offset for postings, positions & offsets.
        bit_packer.write(
